...
double pX = 0;		//  Camera X pos
double pY = 5;		//  Camera Y pos
double pZ = 0;		//  Camera Z pos
...
void display()
{
   ...
   if (mode != 2)
	Print("Angle=%d,%d  Dim=%.1f FOV=%d Projection=%s",th,ph,dim,fov,mode?"Perpective":"Orthogonal");
	   ...
	   Print("X,Y=%.1f,%.1f th=%d ph=%d Projection=Fisrt Person Navigation", pX, pZ, th, ph);
...
void special(int key,int x,int y)
{
   ...
   th %= 360;
   ph %= 360;
   // If in First Person, keep ph between +/-90 degrees
   if (mode == 2)
   {
	   if (ph > 90)
		   ph = 90;
